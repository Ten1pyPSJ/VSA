...
def get_entry_signal(data, next_candle):
    ...
    if signal_strength > 0.5:  # Сильные бычьи сигналы
        final_signal = 'BUY'
    elif signal_strength < -0.5:  # Сильные медвежьи сигналы
        final_signal = 'SELL'
...
if __name__ == '__main__':
    try:
        ...
        # Вывод VSA-признаков
        print("\nVSA-ПРИЗНАКИ:")
        print(f"Спред: {last_candle['spread']:.2f}")
        print(f"Относительный спред: {last_candle['relative_spread']:.2f}")
        print(f"Бычий объем: {last_candle['bull_volume']:.2f} (отношение: {last_candle['bull_volume_ratio']:.2f})")
        print(f"Медвежий объем: {last_candle['bear_volume']:.2f} (отношение: {last_candle['bear_volume_ratio']:.2f})")
        print(f"Сильная свеча: {'Да' if last_candle['is_strong_bar'] else 'Нет'}")
        print(f"Слабая свеча: {'Да' if last_candle['is_weak_bar'] else 'Нет'}")
        print(f"Зона спроса: {'Да' if last_candle['demand_zone'] else 'Нет'}")
        print(f"Зона предложения: {'Да' if last_candle['supply_zone'] else 'Нет'}")
        print(f"Отклонение VWAP: {last_candle['vwap_deviation']:.4f}")
        print(f"Корреляция цена/объем: {last_candle['price_volume_corr']:.4f}")
        print("="*50 + "\n")

        print("\nИНТЕРПРЕТАЦИЯ:")
        if last_candle['is_strong_bar']:
            print("- Сильный бычий сигнал (цена и объем согласованы)")
        if last_candle['demand_zone']:
            print("- Явное преобладание спроса")
        if last_candle['close'] > last_candle['open'] and last_candle['bull_volume_ratio'] > 1:
            print("- Бычий объем подтверждает рост")
        if last_candle['vwap_deviation'] > 0.01:
            print("- Цена значительно выше VWAP (бычий сигнал)")
        if last_candle['price_volume_corr'] > 0.7:
            print("- Сильная положительная корреляция цена/объем (подтверждение тренда)")
